...
$(document).on("ready", function (e) {
	const get_employee = function (element, callback, placeholder) {
		$.ajax({
			type: "GET",
			url: `${site_url}/ajax/select2/abc/employee`,
			dataType: "JSON",
	...
	get_employee($("#param-employee"), create_select2, "Select Employee");

	const dt_employee = $("#table-sales-detail").DataTable({
		ajax: {
			type: "GET",
			data: {
				date_start: function () {
			...
			url: `${site_url}/ajax/abc/report/le_sales_detail`,
			dataSrc: "",
		},
		columnDefs: [
			...
			{ targets: 3, data: "Unit", className: "text-center" },
			{ targets: 4, data: "Qty", className: "text-right" },
			{
				targets: 5,
				data: "401",
			...
			{ targets: 10, data: "Employee", visible: false },
		],
		order: [1, "asc"],
		orderFixed: [10, "asc"],
		autoWidth: false,
	...
	$("#param-employee").on("change", function (e) {
		e.preventDefault();
		employee = $(this).val();
