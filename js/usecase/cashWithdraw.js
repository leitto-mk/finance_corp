...
const dtColumns = [
    ...
    },
    {
        data: response => {
            return `
                <a href="${baseURL}/Cash_adv/edit_ca_withdraw'?docno=${response.DocNo}" target="_blank" type="button" class="btn btn-xs green">
                    <i class="fa fa-edit"> </i>
                </a>
                <a href="${baseURL}/Cash_adv/view_reps_cash_withdraw?docno=${response.DocNo}&branch=${response.Branch}&transdate=${response.TransDate}" target="_blank" name="report" type="button" class="btn btn-xs green-meadow">
...
const caw = {
    indexPage: {
        initDT: () => {
            ...
            repository.generateDataTable('table', 'ajax_get_ranged_ca_withdraw', postData, dtColumns)
        },
        ...
        eventShowList: () => {
            $('#preview, #search').click(function(){
                ...
                repository.generateDataTable('table', 'ajax_get_ranged_ca_withdraw', { docno, date_start, date_end }, dtColumns)
